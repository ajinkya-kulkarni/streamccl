...
def _write_filaments(source: h5py.Dataset, spacing: int) -> int:
    """Write disjoint axis-0 filaments without materializing the full volume."""
    if source.ndim != 3:
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="streamccl-adversarial-", dir=args.workdir) as td:
